...
\begin{itemize}
\item Instead of annotating every single action we put them into a fence
\item a fence is a set of actions where the db node forces all other replicas to execute the actions in the fence and send a receipt
\item Essentially turns the cluster into an array of 'CP' databases until the fence is completed
\end{itemize}
\end{frame}
