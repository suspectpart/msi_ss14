...
\begin{itemize}
\item Amazon Shopping Cart Example
\item Explain Need for Consistency on Demand in certain business cases
\end{itemize}
\end{frame}
...
\begin{itemize}
\item every operation accepted by the db has to annotaed with a "consistency annotation"
\item either ordinary or causal
\item ordinary actions behave like we defined previously
\item causal actions make all operations performed before the annotations visible to other actions
\end{itemize}
\end{frame}
...
\begin{itemize}
\item instead of annotation every single action we put them into a fence
\item a fence is a set of actions where the db node forces all other replica to execute the actions in the fence and send a receipt
\item Essentially turns the cluster into an array of 'CP' databasesCP)
\end{itemize}
\end{frame}
