...
\begin{itemize}
\item Basic Eventual Eonsistency
\item Session Guarantees
\item Causality axioms
\end{itemize}
\item Every form contains multiple axioms
\item More axioms means stronger consistency
\end{itemize}
\end{frame}
...
\begin{itemize}
\item SOwf: \textit{so is the union of transitive, irreflexive and total orders
...
\framesubtitle{Basic Eventual Consistency Axioms}
\begin{itemize}
\item Data Type Axiom: \linebreak \(\forall a \in A. \text{ } \mathrm{rval}(a) = F \mathrm{_{type(a)}} (\mathrm{ctxt}(a))\)
...
\begin{itemize}
\item Every \textbf{action} accepted by the database has to annotaed with a \textbf{consistency annotation}
...
\begin{itemize}
\item Instead of annotating every single action, a \textbf{fence} could be used
